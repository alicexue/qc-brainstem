...
- **Ratings** are saved in the browser as the rater works, under their name, so refreshing or closing the tab loses nothing. They come out as the same two CSVs the Flask apps write, with the same columns (`lc_qc_notes_<name>.csv`, `sn_qc_notes_<name>.csv`).
...
3. Save your ratings as files with **Download LC CSV** and **Download SN CSV**. Each button downloads one file; there are two because browsers often block a page that downloads two files at once. The bar at the top shows how many changes in each file you haven't downloaded yet.
4. To continue on another computer or browser, choose your downloaded CSVs under **Previous ratings** on the start page.
